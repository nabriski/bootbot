  ...
  _handleConversationResponse(type, event) {
    ...
    this._conversations.forEach(convo => {
      if (userId && userId === convo.userId && convo.isActive()) {
        ...
        //get started event - user deleted conversation and restared, terminate conversation and handle it outside
        if(type === "postback"){
            const payload = event.postback.payload;
        ...
        //overrides - get out of conversation and pass it through
        if(type === "message"){
            let text = event.message.text;
            if(convo.overrides && convo.overrides.indexOf(text) > -1){
                convo.end();
                captured = false;
                return;
            }
